...
test_that("clust.bin.pair.test", {
  tests <- c("yang", "durkalski", "obuchowski", "eliasziw")
  
  tc.stats <- sapply(tests, function(test) clust.bin.pair.test(tc$ak, tc$bk, tc$ck, tc$dk, method=test)$statistic)
  expect_equal(c(3.13, 2.32, 2.86, 3.66), unname(round(tc.stats, 2)))

  pc.stats <- sapply(tests, function(test) clust.bin.pair.test(pc$ak, pc$bk, pc$ck,pc$dk, method=test)$statistic)
  expect_equal(c(8.43, 7.54, 7.19, 10.23), unname(round(pc.stats, 2)))
  
  expect_error(clust.bin.pair.test(1, 2, 3, 4, method="xxx"), "method")
})